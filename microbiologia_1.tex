...
D= tempo di riduzione decimale. Tempo in minuti richiesto perché si verifichi una riduzione di 10 volte il numero di cellule viventi presenti. E’ inversamente proporzionale alla T.

(immagine)

Modello di morte microbica – Fattori che influenzano l’efficacia di sterilizzazione
1.Dimensione della popolazione iniziale: popolazioni più ampie richiedono tempi di esposizione maggiori 
2. Composizione della popolazione: le cellule vegetative sono distrutte rapidamente mentre le endospore sono più resistenti. 
3. Influenze ambientali: la presenza di materiali organici (sangue, feci saliva) tende ad inibire l’efficacia. Il calore è più efficace per sostanze con pH acidi (es: per succhi di frutta è sufficiente la pasteurizzazione). 
4. Tempo di esposizione/concentrazione dell’agente sterilizzante: I trattamenti sono più efficaci se effettuati ad alte concentrazioni e per tempi prolungati. 
La sterilizzazione al calore umido – l’autoclave
autoclave: 
• Vapore sotto pressione permette alla temperatura di raggiungere i 121ºC 
• Sufficiente a distruggere le endospore e le cellule vegetative.
...
efficacia = ottima. Resistenza che scalda l’acqua portandola a 100°, quando è piena di vapore si chiude la valvola e la temperatura inizia a salire . aumentata la temperatura l’autoclave conta 20min in cui viene sterilizzato il tutto. Finiti i minuti, annunciati dal segnale acustico, inizia ad abbassarsi la temperatura. Quando si arriva dopo i 100° si fa uscire l’ultimo vapore e si può aprire il portellone.
Nel grafico temperature sono presenti due curve sfalsate: una rappresenta la temperatura dell’autoclave, l’altra dell’oggetto. All’interno dell’autoclave è presente una sonda che viene immesse in una bottiglia


contenente acqua in modo da sapere che temperatura hanno raggiunto i terreni. Sono sfalsate perche la prima viene scaldata l’autoclave (?)
La sterilizzazione al calore
pasteurizzazione: proposta da Pasteur per evitare il deterioramento dei vini dovuto a microrganismi responsabili della fermentazione lattica e acetica. Il procedimento fu poi applicato al latte a fine 1800 riducendo la frequenza di TBC e brucellosi 
• Classica 63-66°C per 30 minuti 
• 71ºC per 15 secondi, metodo HTST (High Temperature, Short Time) 
• UHT (Ultra High Temperature) 140°C per 3 secondi e raffreddamento rapido 
• Riduce significativamente la popolazione microbica, aumenta la conservabilità
controllo dell’efficacia Ampolla di Bacillus stearotermophilus ceppo PA3679 . si può controllare l’efficacia dell’autoclave : si mette dentro una provetta in cui al suo interno è presente un terreno nutrizio con dentro un indicatore ed una striscetta imbevuta di spore di batteri al di fuori, il tutto dentro un’ampolla. dopo l’autoclave si rompe la provetta e se il colore vira da rosso a giallo significa che l’autoclave non ha sterilizzato bene, se invece il colore rimane rosso l’autoclave ha avuto i suoi effetti.
La sterilizzazione mediante radiazioni
Due tipi di radiazioni: raggi g e ultravioletti. UV utilizzati per la sterilizzazione di aria per es di una stanza; raggi g sono molto penetranti e quindi si utilizza per sterilizzare il materiale in laboratorio.

(immagine)

UV scarsa capacità di penetrazione. Usate per sterilizzare stanze o cappe a flusso laminare. Gli UV provocano danni alla cute e danneggiano gli occhi Radiazioni ionizzanti alta capacità di penetrazione. Molto usato per la sterilizzazione di antibiotici, ormoni, fili da sutura, siringhe, plasticheria
La sterilizzazione mediante filtrazione
Metodo usato per soluzioni termolabili quali vitamine, antibiotici zuccheri. Membrane Filtranti: Differenti dimensioni dei pori: 0.22-0.45 $\mu$m: Usato per filtrare i batteri. Non trattiene spirochete, micoplasmi e virus. 0.01 $\mu$m: trattengono i virus e alcune proteine.
Filtri a profondità costituiti da:
• Fibre sovrapposte di diverse sostanze (carta, vetro..) 
• Si può usare come pre filtrazione per rimuovere particelle sospese 
• “azione trappola” 
Filtri Nucleopore:
• Sottili pellicole di policarbonato (spessore $\sim$10 mm) 

• Pori formati da trattamenti chimici “etching” 
• pori relativamente grandi 
• Utili per la microscopia – il materiale filtrato è su un singolo piano sulla superficie
Membrane filtranti convenzionali
• Composti polimerici come acetato di cellulosa o nitrato di cellulosa • Diametro dei pori variabili • “azione tipo setaccio
Disinfezione, antisepsi
Disinfettanti: Agenti chimici che uccidono i microrganismi; usati su oggetti inanimati 
Antisettico: Agente chimico usato per uccidere o inibire la crescita dei microrganismi sufficientemente non tossico per potere essere applicato a tessuti viventi
Quali : Alcool, Aldeidi, formaldeidi, glutaraldeide, ossido di etilene, alogeni(Cl,I), metalli, fenolo, Sali di ammonio quaternario, perossidi.
Agenti batteriostatici, battericidi e batteriolitici
Azione di agenti antimicrobici: 
• microbicida 
• battericida 
• fungicida 
• algicida 
• viricida 
• Agenti possono variare a seconda della loro tossicità selettiva
